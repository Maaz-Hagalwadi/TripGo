...
import { createPremiumPaymentIntent } from '../../../api/paymentService';

const stripePromise = STRIPE_PUBLISHABLE_KEY ? loadStripe(STRIPE_PUBLISHABLE_KEY) : null;

const PLANS = [
  { id: 'monthly',   label: 'Monthly',   price: '₹299',   per: 'per month',    savings: null,          popular: false },
  { id: 'quarterly', label: 'Quarterly', price: '₹749',   per: 'per 3 months', savings: 'Save ₹148',   popular: true  },
  { id: 'yearly',    label: 'Yearly',    price: '₹2,499', per: 'per year',     savings: 'Save ₹1,089', popular: false },
];
...
const PremiumCheckoutForm = ({ plan, onSuccess, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (submitError) { setError(submitError.message || 'Payment details incomplete.'); return; }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex gap-3">
        ...
        <button type="submit" disabled={!stripe || !elements || !ready || submitting}
          className="flex-1 py-3 rounded-xl bg-[#002046] text-white text-sm font-black hover:bg-[#003a80] disabled:opacity-60 transition-colors">
          {submitting ? 'Processing...' : `Pay ${PLANS.find(p => p.id === plan)?.price}`}
        </button>
      </div>
    </form>
  );
...
const PaymentModal = ({ plan, clientSecret, onSuccess, onClose }) => (
  <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
    <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
      <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
        <div>
          <p className="font-bold text-slate-900">Complete Payment</p>
          <p className="text-xs text-slate-500 mt-0.5">TripGo Premium — {PLANS.find(p => p.id === plan)?.label}</p>
        </div>
      ...
      <div className="p-5">
        {clientSecret && stripePromise ? (
          <Elements stripe={stripePromise} options={{ clientSecret }}>
            <PremiumCheckoutForm plan={plan} onSuccess={onSuccess} onClose={onClose} />
          </Elements>
        ) : (
          <p className="text-sm text-slate-500">Payment unavailable. Please try again.</p>
        )}
      </div>
    </div>
  </div>
...
const UserPremium = () => {
  ...
  const handleSubscribe = async () => {
    ...
    setSubscribing(true);
    try {
      const { clientSecret: secret } = await createPremiumPaymentIntent(selectedPlan);
  ...
  const handlePaymentSuccess = () => {
    ...
    toast.success('TripGo Premium activated! Enjoy your benefits.');
  };
        ...
        <div className="rounded-2xl bg-gradient-to-br from-[#002046] via-[#003a80] to-[#001224] p-8 text-white shadow-sm relative overflow-hidden">
          ...
          {user?.isPremium && (
            <div className="mt-4 inline-flex items-center gap-2 bg-white/15 rounded-xl px-4 py-2">
              ...
              <span className="text-sm font-bold">You are a Premium member</span>
            </div>
          )}
          {!user?.isPremium && (
        ...
        {!user?.isPremium && (
          <div className="rounded-2xl bg-white ring-1 ring-slate-200 shadow-sm overflow-hidden">
              ...
              {PLANS.map((p) => (
                <button
                  ...
                  className={`w-full flex items-center justify-between gap-4 rounded-2xl px-5 py-4 ring-2 transition-all text-left ${
                    selectedPlan === p.id ? 'ring-[#002046] bg-[#002046]/[0.04]' : 'ring-slate-200 hover:ring-slate-300 bg-white'
                  }`}
                >
            ...
            <div className="px-5 pb-5">
              ...
              >
                {subscribing ? 'Preparing payment...' : `Subscribe to ${plan?.label} — ${plan?.price}`}
              </button>
